...
class WarnetConnect {
    ...
    public void biayaTotal() {
        ...
        double biayaPengetikan;
        double biayaGame; 

        if (this.lamaInternet < 30 && this.lamaInternet != 0) {
            biayaInternet = 2000; 
        } else {
            biayaInternet = ((double)this.lamaInternet / 60) * 4000;
        }

        if (this.lamaPengetikan < 30 && this.lamaPengetikan != 0) {
            biayaPengetikan = 1000;
        } else {
            biayaPengetikan = ((double)this.lamaPengetikan / 60) * 2000;
        }

        if (this.lamaGame < 30 && this.lamaGame != 0) {
            biayaGame = 2500; 
        } else {
            biayaGame = ((double)this.lamaGame / 60) * 5000;
        }
